...
import net.raphimc.viabedrock.api.util.BedrockLineBreaks;
...
public class SignBlockEntityRewriter implements BlockEntityRewriter.Rewriter {
    ...
    private CompoundTag translateText(final CompoundTag bedrockText) {
        ...
        // LOCAL PATCH (endstone-proxy): a break can arrive as a real \n, as \r\n, or as the two
        // characters \ and n left escaped by a JSON rawtext pipeline. Only the first survives the
        // splits below, which is why signs written by addons arrived with every line on line one.
        final String text = BedrockLineBreaks.normalize(bedrockText.getString("Text", ""));
