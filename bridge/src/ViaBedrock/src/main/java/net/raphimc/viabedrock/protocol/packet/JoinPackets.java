...
public class JoinPackets {
    ...
    private static void handleJavaClientGameJoin(final UserConnection user) {
        ...
        // LOCAL PATCH (endstone-proxy): the tab list header and footer are configurable, and default to
        // the server's own level name and nothing else. Upstream advertises ViaBedrock and its GitHub
        // link in the footer, which is reasonable for a standalone translator and wrong for a server
        // whose players have no idea what ViaBedrock is. Set endstone.bridge.tabListHeader /
        // endstone.bridge.tabListFooter to put your own text there; § colour codes work.
        final PacketWrapper tabList = PacketWrapper.create(ClientboundPackets26_1.TAB_LIST, user);
        tabList.write(Types.TAG, TextUtil.stringToNbt(
                System.getProperty("endstone.bridge.tabListHeader", joinGameStorage.levelName() + "\n"))); // header
        tabList.write(Types.TAG, TextUtil.stringToNbt(
                System.getProperty("endstone.bridge.tabListFooter", ""))); // footer
