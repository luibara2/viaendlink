    ...
    public static void register(final BedrockProtocol protocol) {
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.SET_TITLE, null, wrapper -> {
            ...
            try {
                ...
                // LOCAL PATCH (endstone-proxy): Java draws titles, subtitles and the action bar with a
                // single un-wrapped call, so a newline there is not a line break — at best it renders
                // as nothing, at worst as a missing glyph, and the words run together. Bedrock allows
                // multi-line text in all three. Nothing can make Java draw two lines, so the breaks are
                // normalised (including the escaped two-character form a rawtext pipeline leaves
                // behind) and collapsed to a separator, which keeps the text readable instead of merged.
                switch (type) {
                    case Title, TitleTextObject, Subtitle, SubtitleTextObject, Actionbar, ActionbarTextObject ->
                            text = BedrockLineBreaks.flatten(text, System.getProperty("endstone.bridge.titleLineSeparator", " "));
                    default -> {
                    }
                }
