...
public class ExternalInterface {
    ...
    public static void fillPlayerData(final ProxyConnection proxyConnection) {
        ...
        try {
            ...
            // LOCAL PATCH (endstone-proxy): when embedded in the proxy and joining a Bedrock target
            // with no account configured, supply the login token ourselves rather than letting
            // ViaBedrock self-sign a bare one. It carries the player's real IP and a shared secret
            // that proves the login came from this ViaProxy. Inert when run standalone.
            if (proxyConnection.getServerVersion().equals(BedrockProtocolVersion.bedrockLatest)
                    && !proxyConnection.getUserConnection().has(AuthData.class)
                    && EndstoneBridgeAuth.isEnabled()) {
                EndstoneBridgeAuth.fill(proxyConnection);
            }
